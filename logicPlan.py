...
def sentence2() -> Expr:
    ...
    return conjoin([expr_1, expr_2, expr_3, expr_4])
...
def sentence3() -> Expr:
    ...
    sent1 = PacmanAlive_1 % ((PacmanAlive_0 & (~PacmanKilled_0)) | ((~PacmanAlive_0) & PacmanBorn_0))
...
def atLeastOne(literals: List[Expr]) -> Expr:
    ...
    return disjoin(clauses) # disjoint & 1
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    one_or_more = atLeastOne(literals)
    one_or_zero = atMostOne(literals)

    return conjoin([one_or_zero, one_or_more]) # (one | zero) & (one | more) = one, since zero and more are mutually exclusive
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    current = PropSymbolExpr(pacman_str, x, y, time=now) # current position
    actions = disjoin(possible_causes) # possible action we could have taken to get to current (only one is True)
    LHS = current
    RHS = actions
    return LHS % RHS
    util.raiseNotDefined()
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for coord in all_coords:
        # if wall(x,y) then ~pacman(x,y,t)
        pacphysics_sentences.append(PropSymbolExpr(wall_str, coord[0], coord[1]) >> ~PropSymbolExpr(pacman_str, coord[0], coord[1], time=t))
    # pacman is exactly one of the squares at timestep t
    tempList = []
    for coord in non_outer_wall_coords:
        tempList.append(PropSymbolExpr(pacman_str, coord[0], coord[1], time=t))
    pacphysics_sentences.append(exactlyOne(tempList))
    # Pacman takes exactly one action at timestep t
    tempList = []
    for action in DIRECTIONS:
        tempList.append(PropSymbolExpr(action, time=t))
    pacphysics_sentences.append(exactlyOne(tempList))
    # Results of calling sensorModel(...), unless None
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    model1 = findModel(conjoin([conjoinedKB, PropSymbolExpr(pacman_str, x1, y1, time=1)]))
    model2 = findModel(conjoin([conjoinedKB, ~PropSymbolExpr(pacman_str, x1, y1, time=1)]))
...
def positionLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # pacman's initial position is x0, y0
    KB.append(PropSymbolExpr(pacman_str,x0, y0, time=0))
    for coord in all_coords:
        if coord != (x0, y0):
            KB.append(~PropSymbolExpr(pacman_str, coord[0], coord[1], time=0))
    # assign walls
    i = j = 0
    for wall_row in walls_grid:
    ...
    for t in range(50):
        ...
        # pacman can only be in only one location in non_wall_coords at timestep t
        for coord in all_coords:
            # if wall(x,y) then pacman not at (x,y)
            KB.append(PropSymbolExpr(wall_str, coord[0], coord[1]) >> ~PropSymbolExpr(pacman_str, coord[0], coord[1], time=t))
        # pacman can only be in one location in non_wall_coords at timestep t
        tempList = []
        for coord in non_wall_coords:
            tempList.append(PropSymbolExpr(pacman_str, coord[0], coord[1], time=t))
        KB.append(exactlyOne(tempList))

        # Check if KB is satisfiable
        KBconjoined = conjoin(KB)
        GoalAssertionList = [PropSymbolExpr(pacman_str, xg, yg, time=t)]
        for coord in non_wall_coords:
            if coord != (xg, yg):
                GoalAssertionList.append(~PropSymbolExpr(pacman_str, coord[0], coord[1], time=t))
        GoalAssertion = conjoin(GoalAssertionList)
        model = findModel(conjoin([KBconjoined, GoalAssertion]))
        if model:
            return extractActionSequence(model, actions) # if model is satisfiable return actions to get to goal

        # pacman can take only one action at timestep t
        tempList = []
        ...
        for coord in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(coord[0], coord[1], t+1, walls_grid))
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    # assign food at time 0
    for coord in food:
        # if coord in food append food[x,y]_t:
        KB.append(PropSymbolExpr(food_str, coord[0], coord[1], time=0))
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            tempList.append(PropSymbolExpr(pacman_str, x, y, time=t))
        KB.append(exactlyOne(tempList))

        # Check if goal is satisfied
        KBconjoined = conjoin(KB)
        GoalAssertionList = []
        # for food coord there is no food
        for x, y in food:
            GoalAssertionList.append(~PropSymbolExpr(food_str, x, y, time=t))
        GoalAssertion = conjoin(GoalAssertionList)
        model = findModel(conjoin([KBconjoined, GoalAssertion]))
        if model:
            return extractActionSequence(model, actions)  # if model is satisfiable return actions to get to goal
        ...
        #### Food Axiom
        for x, y in food:
            # pacman[x,y]_t+1 | ~food[x,y]_t >> ~food[x,y]_t+1
            KB.append((PropSymbolExpr(pacman_str, x, y, time=t+1) | ~PropSymbolExpr(food_str, x, y, time=t))
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            ...
            KBconjoined = conjoin(KB)
            model = findModel(conjoin([KBconjoined, pos]))
            if model:
                possible_locations.append((x,y))
            # pacman is provably at x, y
            if entails(KBconjoined, pos):
                KB.append(pos)
            # pacman is provably not at x, y
            if entails(KBconjoined, ~pos):
                KB.append(~pos)
...
def mapping(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # pacman's initial position is x0, y0 (with obviously no wall)
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y, in non_outer_wall_coords:
            ...
            conjoinedKB = conjoin(KB)
            # wall is provably at x, y
            if entails(conjoinedKB, wall):
                known_map[x][y] = 1
                KB.append(wall)
            # wall is provably not at x, y
            if entails (conjoinedKB, ~wall):
                KB.append(~wall)
                known_map[x][y] = 0
            # agent next action
